...
type ThingsWebSocketConnection struct {
	...
	obsEventsDelete []chan *WSMessage
	obsEventsCreate []chan *WSMessage
	obsEventsUpdate []chan *WSMessage
}
...
func (c *ThingsWebSocketConnection) ObserveUpdateEvents(ch chan *WSMessage) (err error) {
	if err = c.startSendEvents(); err != nil {
		return
	}

	c.obsEventsUpdate = append(c.obsEventsUpdate, ch)
	return
}

func (c *ThingsWebSocketConnection) ObserveCreatedEvents(ch chan *WSMessage) (err error) {
	if err = c.startSendEvents(); err != nil {
		return
	}

	c.obsEventsCreate = append(c.obsEventsCreate, ch)
	return
}

func (c *ThingsWebSocketConnection) ObserveDeletedEvents(ch chan *WSMessage) (err error) {
	if err = c.startSendEvents(); err != nil {
		return
	}

	c.obsEventsDelete = append(c.obsEventsDelete, ch)
	return
}
